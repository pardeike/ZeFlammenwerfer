	...
	public class BGCcCursor : BGCcWithMath
	{
		...
		public TR Lerp<T, TR>(string fieldName, Func<T, T, float, TR> lerpFunction)
		{
			if (Curve.PointsCount == 0)
				return lerpFunction(default, default, 0);

			var t = GetAdjacentFieldValues(fieldName, out var from, out
			T to);
		...
		public Quaternion LerpQuaternion(string fieldName, Func<Quaternion, Quaternion, float, Quaternion> customLerp = null)
		{
			...
			if (from.x == 0 && from.y == 0 && from.z == 0 && from.w == 0)
				from = Quaternion.identity;
			if (to.x == 0 && to.y == 0 && to.z == 0 && to.w == 0)
		...
		{
			var t = GetTForLerp(out var indexFrom, out var indexTo);
