...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcCursorChangeLinear : BGCcWithCursor
	{
		...
		[SerializeField]
		[Tooltip("Constant movement speed along the curve (Speed * Time.deltaTime)." +
										  "You can override this value for each point with speedField")]
		private float speed = 5;

		[SerializeField][Tooltip("How to change speed, then curve's end reached.")] private OverflowControlEnum overflowControl;

		[SerializeField]
		[Tooltip("If curve's length changed, " +
										  "cursor position be adjusted with curve's length to ensure visually constant speed along the curve. ")]
		...
		[SerializeField][Tooltip("Event is fired, then point is reached")] private readonly PointReachedEvent pointReachedEvent = new();
		...
		private void Step()
		{
			if (Stopped || (speedField == null && Mathf.Abs(speed) < SpeedThreshold))
				return;
			...
			if (isPlaying && delayStarted >= 0 && !CheckIfDelayIsOver(math, cursor))
				return;
			...
			if (adjustByTotalLength)
			{
				...
				if (Math.Abs(newLength) > BGCurve.Epsilon && Math.Abs(oldLength) > BGCurve.Epsilon && Math.Abs(newLength - oldLength) > BGCurve.Epsilon)
					distance = distance * newLength / oldLength;
			}
			...
			if ((checkDelay || firingEvents) && CheckForNewDelay(math, distance, ref newSectionIndex, checkDelay, firingEvents))
				return;
			...
			//--------------------------------  Check Overflows
			if (newDistance < 0 || newDistance > math.GetDistance())
				Overflow(math, ref newDistance, currentSpeed >= 0, checkDelay, firingEvents);
		...
		private bool CheckDelayAtSectionChanged(int newSectionIndex, bool checkDelay, bool firingEvents, bool speedPositive)
		{
			...
			else
			{
				...
				if (currentSectionIndex == 0 && !Curve.Closed)
					currentSectionIndex = lastPointIndex;
		...
		{
			PointReached?.Invoke(this, PointReachedArgs.GetInstance(pointIndex));
		...
		{
			private static readonly PointReachedArgs Instance = new();
