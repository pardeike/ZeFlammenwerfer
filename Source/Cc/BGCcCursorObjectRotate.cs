...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcCursorObjectRotate : BGCcWithCursorObject
	{
		...
		public void Update()
		{
			...
			if (!TryToCalculateRotation(ref rotation))
				return;

			targetTransform.rotation = rotation;

			ChangedObjectRotation?.Invoke(this, null);
		}
		...
		public bool TryToCalculateRotation(ref Quaternion result)
		{
			...
			else
			{
				// =============================================== By field
				if (pointsCount == 1)
					result = Curve[0].GetQuaternion(rotationField.FieldName);
				else
				{
					if (revolutionsAroundTangentField == null && revolutionsAroundTangent == 0)
						result = LerpQuaternion(rotationField.FieldName); //no compications
					else
					{
						...
						if (additionalRevolutions > 0 && math.IsCalculated(BGCurveBaseMath.Field.Tangent))
						{
							...
							if (Vector3.SqrMagnitude(tangent) > 0.01)
							{
								...
								//change if Clockwise
								if (revolutionsClockwiseField != null ? Curve[currentSection].GetBool(revolutionsClockwiseField.FieldName) : revolutionsClockwise)
									targetAngle = -targetAngle;

								//calculate t ratio for lerping. 0- at start (from point), 1= at the end (to point)
								var t = GetT(out var _, out var _, currentSection);
								var angle = Mathf.Lerp(0, targetAngle, t);

								//sum up rotaitons
								result *= Quaternion.AngleAxis(angle, tangent);
							}
						}
					}
				}
