...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcMath : BGCc, IBGCurveMathI
	{
		//===============================================================================================
		//                                                    Static 
		//===============================================================================================
		...
		public readonly MathChangedEvent mathChangedEvent = new();
		...
		private void InitMath(object sender, EventArgs e)
		{
			...
			if (NewMathRequired)
			{
				...
				math = MathType switch
				{
					MathTypeEnum.Base => new BGCurveBaseMath(Curve, config),
					_ => new BGCurveAdaptiveMath(Curve, (BGCurveAdaptiveMath.ConfigAdaptive)config),//                    case MathTypeEnum.Adaptive:
				};
		...
		private void MathOnChangeRequested(object sender, EventArgs eventArgs)
		{
			...
			if (visibilityCheck == null || visibilityCheck.Visible)
				return;
					...
					for (var i = 0; i < pointsCount; i++)
					{
						...
						if (point.ControlType != BGCurvePoint.ControlTypeEnum.Absent)
						{
							if (closed || i != 0)
							{
								...
								//copy/pasted for the sake of performance
								if (min.x > controlWorld.x)
									min.x = controlWorld.x;
								if (min.y > controlWorld.y)
									min.y = controlWorld.y;
								if (min.z > controlWorld.z)
									min.z = controlWorld.z;

								if (max.x < controlWorld.x)
									max.x = controlWorld.x;
								if (max.y < controlWorld.y)
									max.y = controlWorld.y;
								if (max.z < controlWorld.z)
		...
		private sealed class VisibilityCheck : MonoBehaviour
		{
			...
			{
				Visible = true;
				BecameVisible?.Invoke(this, null);
			}
