...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcSweep2D : BGCcSplitterPolyline
	{
		...
		private ProfileModeEnum profileMode = ProfileModeEnum.Line;
		...
		*/
		...
		public override string Error
		{
			get
			{
				return ChoseMessage(base.Error, () =>
				{
					...
					if (PointsCount * profilePointsCount > 65534)
		...
		//===============================================================================================
		private static readonly List<PositionWithU> crossSectionList = new();
		...
		public void UpdateUI()
		{
			...
			else
			{
				...
				for (var i = 0; i < points.Count; i++)
					crossSectionList.Add(new PositionWithU { Position = points[i] });
			}
			var crossSectionCount = crossSectionList.Count;
			var crossSectionDistance = .0f;
			for (var i = 0; i < crossSectionCount - 1; i++)
				crossSectionDistance += Vector3.Distance(crossSectionList[i].Position, crossSectionList[i + 1].Position);
			...
			//------------- normal
			var normal = Curve.Mode2D switch
			{
				BGCurve.Mode2DEnum.XY => swapNormals ? Vector3.back : Vector3.forward,
				BGCurve.Mode2DEnum.XZ => swapNormals ? Vector3.down : Vector3.up,
				BGCurve.Mode2DEnum.YZ => swapNormals ? Vector3.left : Vector3.right,
				_ => throw new ArgumentOutOfRangeException("Curve.Mode2D"),
			};
			...
			if (closed)
			{
				...
				firstTangent = positions[1] - positions[0];
