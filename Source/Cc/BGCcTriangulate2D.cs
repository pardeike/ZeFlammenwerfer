...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcTriangulate2D : BGCcSplitterPolyline
	{
		...
		[SerializeField][Tooltip("UV scale")] private Vector2 scaleUV = new(1, 1);
		[SerializeField][Tooltip("UV offset")] private Vector2 offsetUV = new(0, 0);
		[SerializeField][Tooltip("Flip triangles")] private bool flip;
		[SerializeField][Tooltip("Double sided")] private bool doubleSided;
		[SerializeField][Tooltip("UV scale for back side")] private Vector2 scaleBackUV = new(1, 1);
		[SerializeField][Tooltip("UV offset for back side")] private Vector2 offsetBackUV = new(0, 0);
		...
		public Vector2 ScaleUv
		{
			...
			set
			{
				if (Mathf.Abs(scaleUV.x - value.x) < BGCurve.Epsilon && Mathf.Abs(scaleUV.y - value.y) < BGCurve.Epsilon)
					return;
		...
		public bool UpdateEveryFrame
		{
			...
			set
			{
				...
				if (updateEveryFrame && !everyFrameUpdateIsRunning && gameObject.activeSelf && Application.isPlaying)
		...
		public override void Start()
		{
			...
			if (updateEveryFrame && gameObject.activeSelf && Application.isPlaying)
				StartCoroutine(UiUpdater());
		}
