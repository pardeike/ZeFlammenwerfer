...
namespace BansheeGz.BGSpline.Components
{
	...
	public class BGCcTrs : BGCcCursor
	{
		...
		private void Step()
		{
			if (cursorChangeMode == CursorChangeModeEnum.Constant && System.Math.Abs(speed) < BGCurve.Epsilon)
				return;
			if (Curve.PointsCount < 2 || Error != null)
				return;
			...
			switch (cursorChangeMode)
			{
				...
				case CursorChangeModeEnum.LinearField:
					...
					break;
				case CursorChangeModeEnum.LinearFieldInterpolate:
					IBGCurvePointI fromPoint;
					IBGCurvePointI toPoint;
					float ratio;
		...
		public void Trs(int sectionIndex = -1)
		{
			...
			//move
			if (moveObject)
				objectToManipulate.position = Math.CalcPositionByDistance(distance);
			...
			if (rotateObject)
			{
				...
				{
					var up = upVector switch
					{
						RotationUpEnum.WorldUp => Vector3.up,
						RotationUpEnum.WorldDown => Vector3.down,
						RotationUpEnum.WorldRight => Vector3.right,
						RotationUpEnum.WorldLeft => Vector3.left,
						RotationUpEnum.WorldForward => Vector3.forward,
						RotationUpEnum.WorldBack => Vector3.back,
						_ => throw new ArgumentOutOfRangeException("upVector"),
					};
				...
				{
					rotation *= Quaternion.Euler(offsetAngle);

					// interpolation (rotation speed)
					ObjectToManipulate.rotation = rotationInterpolation switch
					{
						RotationInterpolationEnum.None => rotation,
						RotationInterpolationEnum.Lerp => Quaternion.Lerp(ObjectToManipulate.rotation, rotation, lerpSpeed * Time.deltaTime),
						RotationInterpolationEnum.Slerp => Quaternion.Slerp(ObjectToManipulate.rotation, rotation, slerpSpeed * Time.deltaTime),
		...
		{
			FillInterpolationInfo(ref sectionIndex, out var fromPoint, out var toPoint, out var ratio);
		...
		private void FillInterpolationInfo(ref int sectionIndex, out IBGCurvePointI fromPoint, out IBGCurvePointI toPoint, out float ratio)
		{
			if (sectionIndex == -1)
				sectionIndex = CalculateSectionIndex();
