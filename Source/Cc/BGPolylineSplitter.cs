		...
		public abstract class PositionsProvider
		{
			...
			public virtual void Build(List<Vector3> positions, int straightLinesCount, bool[] straightBits, List<BGCcSplitterPolyline.PolylinePoint> points)
			{
				...
				for (var i = 0; i < sectionsCount; i++)
				{
					...
					if (DistanceMinConstrained && section.DistanceFromEndToOrigin < DistanceMin)
						continue;
					if (DistanceMaxConstrained && section.DistanceFromStartToOrigin > DistanceMax)
						continue;
					...
					if (straightLinesCount != 0 && straightBits[i])
					{
						...
						if (DistanceMinConstrained && positions.Count == 0)
							AddFirstPointIfNeeded(positions, section, lastPoint, prevPoint, points);
						if (DistanceMaxConstrained && !LastPointAdded && lastPoint.DistanceToSectionStart + section.DistanceFromStartToOrigin > DistanceMax
							 && AddLastPointIfNeeded(positions, section, lastPoint, prevPoint, points))
							break;
					...
					// curved line
						...
						FillInSplitSection(section, positions, points);
				}
			}
			...
			protected void FillIn(BGCurveBaseMath.SectionInfo section, List<Vector3> result, int parts, List<BGCcSplitterPolyline.PolylinePoint> points)
			{
				...
				for (var j = 1; j <= parts; j++)
				{
					...
					section.CalcByDistance(currentDistance, out var pos, out var tangent, true, calculatingTangents);
					result.Add(pos);
			...
			public override void Build(List<Vector3> positions, int straightLinesCount, bool[] straightBits, List<BGCcSplitterPolyline.PolylinePoint> points)
			{
				...
				if (parts < sectionsCount)
				{
					...
					if (parts == 1)
					{
						...
						if (curve.Closed)
						{
							var pos = Math.CalcByDistanceRatio(.5f, out var tangent);
							positions.Add(pos);
					...
					else if (parts == 2 && curve.Closed)
					{
						...
						var pos2 = Math.CalcByDistanceRatio(distanceRatio2, out var tangent2);
						positions.Add(pos2);
						points.Add(new BGCcSplitterPolyline.PolylinePoint(pos2, distance * distanceRatio2, tangent2));

						var distanceRatio3 = 2 / 3f;
						var pos3 = Math.CalcByDistanceRatio(distanceRatio3, out var tangent3);
						positions.Add(pos3);
					...
					else
					{
						for (var i = 0; i <= parts; i++)
						{
							...
							var pos = Math.CalcByDistanceRatio(ratio, out var tangent);
							positions.Add(pos);
