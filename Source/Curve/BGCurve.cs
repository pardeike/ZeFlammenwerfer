...
namespace BansheeGz.BGSpline.Curve
{
	...
	public class BGCurve : MonoBehaviour
	{
		...
		private static readonly IBGCurvePointI[] pointArray = new IBGCurvePointI[1];

		//static reusable list for storing Unity objects
		private static readonly List<IBGCurvePointI> pointsList = new();

		//static reusable list for storing points index
		private static readonly List<int> pointsIndexesList = new();
...
#pragma warning restore 0414
		...
		public delegate void IterationCallback(IBGCurvePointI point, int index, int count);
		...
		public IBGCurvePointI[] Points
		{
			get
			{
				return pointsMode switch
				{
					PointsModeEnum.Inlined => points,
					PointsModeEnum.Components => pointsComponents,
					PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => pointsGameObjects,
					_ => throw new ArgumentOutOfRangeException("pointsMode"),
				};
			}
		}
		...
		public int PointsCount
		{
			get
			{
				return pointsMode switch
				{
					PointsModeEnum.Inlined => points.Length,
					PointsModeEnum.Components => pointsComponents.Length,
					PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => pointsGameObjects.Length,
					_ => throw new ArgumentOutOfRangeException("pointsMode"),
				};
			}
		}
		...
		public float SnapDistance
		{
			...
			set
			{
				if (Math.Abs(snapDistance - value) < Epsilon)
					return;
		...
		private List<BGCurveChangedArgs> ChangeList
		{
			get { return changeList ??= new List<BGCurveChangedArgs>(); }
		}
		...
		public int IndexOf(IBGCurvePointI point)
		{
		...
		public IBGCurvePointI AddPoint(BGCurvePoint point)
		{
			return AddPoint(point, PointsCount, null);
		}

		/// <summary>Add a point at specified index</summary>
		public IBGCurvePointI AddPoint(BGCurvePoint point, int index)
		{
		...
		public void Delete(IBGCurvePointI point)
		{
		...
		public void Delete(IBGCurvePointI[] points)
		{
			Delete(points, null);
		}
		...
		public void Reverse()
		{
			...
			for (var i = 0; i < mid; i++)
			{
				...
				if (hasFields)
					SetFieldsValues(point2, pointsMode, GetFieldsValues(point1, pointsMode));
				...
				if (hasFields)
					SetFieldsValues(point1, pointsMode, fields2);
			}
			...
			if (pointsCount % 2 != 0)
			{
				...
				(point.ControlSecondLocal, point.ControlFirstLocal) = (point.ControlFirstLocal, point.ControlSecondLocal);
			}
		...
		public IBGCurvePointI this[int i]
		{
			get
			{
				return pointsMode switch
				{
					PointsModeEnum.Inlined => points[i],
					PointsModeEnum.Components => pointsComponents[i],
					PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => pointsGameObjects[i],
					_ => throw new ArgumentOutOfRangeException("pointsMode"),
				};
			}
			set
		...
		public BGCurvePointField GetField(string name)
		{
			...
			{
				var field = fields[i];
				if (string.Equals(field.FieldName, name))
					return field;
			}
			return null;
		}
		...
		public void Apply2D(Mode2DEnum value)
		{
			...
			if (mode2D != Mode2DEnum.Off && PointsCount > 0)
			{
				...
				FireChange(BGCurveChangedArgs.GetInstance(this, BGCurveChangedArgs.ChangeTypeEnum.Points, Event2D));
		}

		/// <summary>change point's coordinate to comply to 2d mode</summary>
		public virtual void Apply2D(IBGCurvePointI point)
		{
		...
		{
			return mode2D switch
			{
				Mode2DEnum.XY => new Vector3(point.x, point.y, 0),
				Mode2DEnum.XZ => new Vector3(point.x, 0, point.z),
				Mode2DEnum.YZ => new Vector3(0, point.y, point.z),
				_ => point,
			};
		}
		...
		public bool ApplySnapping()
		{
			...
			for (var i = 0; i < length; i++)
				result |= ApplySnapping(points[i]);
			return result;
		}

		/// <summary>apply snapping to one single point</summary>
		public bool ApplySnapping(IBGCurvePointI point)
		{
			if (snapType == SnapTypeEnum.Off)
				return false;
		...
		public bool ApplySnapping(ref Vector3 pos)
		{
			...
			//assign direction
			var direction = snapAxis switch
			{
				SnapAxisEnum.Y => Vector3.up,
				SnapAxisEnum.X => Vector3.right,
				_ => Vector3.forward,//                case SnapAxisEnum.Z:
			};
			...
			for (var k = 0; k < 2; k++)
			{
				...
				if (!Physics.Raycast(ray, out var hit, snapDistance, snapLayerMask, snapTriggerInteraction))
					continue;

				if (!(minDistance < 0) && !(minDistance > hit.distance))
					continue;
			...
			if (snapToBackFaces)
			{
				for (var j = 0; j < 2; j++)
				{
					...
					for (var i = 0; i < count; i++)
					{
						...
						if (!(minDistance < 0) && !(minDistance > distanceToCheck))
							continue;
		...
		{
			if (eventMode == EventModeEnum.NoEvents || transactionLevel > 0 || BeforeChange == null)
				return;
		...
		{
			if (eventMode == EventModeEnum.NoEvents || Changed == null)
				return;
		...
		protected void Update()
		{
			...
			if (eventMode != EventModeEnum.Update)
				return;
			if (snapMonitoring && snapType != SnapTypeEnum.Off)
				SnapIt();
			if (Changed == null)
				return;
			FireFinalEvent();
		...
		public static T[] Insert<T>(T[] oldArray, int index, T[] newElements)
		{
			...
			if (index < oldArray.Length)
				Array.Copy(oldArray, index, newArray, index + newElements.Length, oldArray.Length - index);
		...
		private IBGCurvePointI AddPoint(BGCurvePoint point, int index, Func<IBGCurvePointI> provider = null)
		{
			...
			if (FieldsCount > 0)
			{
				...
				foreach (var field in fields)
					BGCurvePoint.PrivateFieldAdded(field, fieldValues);
			}
		...
		private BGCurvePoint.FieldsValues GetFieldsValues(IBGCurvePointI point, PointsModeEnum pointsMode)
		{
			var fieldValues = pointsMode switch
			{
				PointsModeEnum.Inlined => ((BGCurvePoint)point).PrivateValuesForFields,
				PointsModeEnum.Components => ((BGCurvePointComponent)point).Point.PrivateValuesForFields,
				PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => ((BGCurvePointGO)point).PrivateValuesForFields,
				_ => throw new ArgumentOutOfRangeException("pointsMode"),
			};
			return fieldValues;
		}
		...
		private void SetFieldsValues(IBGCurvePointI point, PointsModeEnum pointsMode, BGCurvePoint.FieldsValues fieldsValues)
		{
			switch (pointsMode)
			{
				case PointsModeEnum.Inlined:
		...
		private void AddPoints(BGCurvePoint[] points, int index, bool skipFieldsProcessing = false, Func<IBGCurvePointI> provider = null)
		{
			if (points == null)
				return;
		...
		private void Delete(int index, Action<IBGCurvePointI> destroyer)
		{
			if (index < 0 || index >= PointsCount)
				throw new UnityException("Unable to remove a point. Invalid index: " + index);

			pointArray[0] = pointsMode switch
			{
				PointsModeEnum.Inlined => points[index],
				PointsModeEnum.Components => pointsComponents[index],
				PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => pointsGameObjects[index],
				_ => throw new ArgumentOutOfRangeException("pointsMode"),
			};
		...
		private void Delete(IBGCurvePointI[] pointsToDelete, Action<IBGCurvePointI> destroyer)
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (indexToRemove > cursor)
					Array.Copy(oldPoints, cursor, newPoints, cursor - i, indexToRemove - cursor);
			...
			if (cursor < oldPoints.Length)
				Array.Copy(oldPoints, cursor, newPoints, cursor - count, oldPoints.Length - cursor);
		...
		private void ConvertPoints(PointsModeEnum pointsMode, Func<IBGCurvePointI> provider = null, Action<IBGCurvePointI> destroyer = null)
		{
			var oldMode = PointsMode;
			if (oldMode == pointsMode)
				return;
						...
						switch (pointsMode)
						{
							case PointsModeEnum.Components:
							{
								...
								for (var i = 0; i < points.Length; i++)
									toAdd[i] = (BGCurvePointComponent)Convert(points[i], oldMode, pointsMode, provider);
							...
							case PointsModeEnum.GameObjectsTransform:
							{
								...
								for (var i = 0; i < points.Length; i++)
									toAdd[i] = (BGCurvePointGO)Convert(points[i], oldMode, pointsMode, provider);
					...
					if (hasAnyPoint)
					{
						...
						if (oldMode == PointsModeEnum.Components)
						{
							...
							switch (pointsMode)
							{
								case PointsModeEnum.Inlined:
									...
									for (var i = 0; i < pointsComponents.Length; i++)
										pointsToAdd[i] = (BGCurvePoint)Convert(pointsComponents[i], oldMode, pointsMode, provider);
								...
								case PointsModeEnum.GameObjectsTransform:
									...
									for (var i = 0; i < pointsComponents.Length; i++)
										toAdd[i] = (BGCurvePointGO)Convert(pointsComponents[i], oldMode, pointsMode, provider);
						...
						else
						{
							...
							switch (pointsMode)
							{
								case PointsModeEnum.Inlined:
									...
									for (var i = 0; i < pointsGameObjects.Length; i++)
										pointsToAdd[i] = (BGCurvePoint)Convert(pointsGameObjects[i], oldMode, pointsMode, provider);
								...
								case PointsModeEnum.Components:
									...
									for (var i = 0; i < pointsGameObjects.Length; i++)
										toAdd[i] = (BGCurvePointComponent)Convert(pointsGameObjects[i], oldMode, pointsMode, provider);
								...
								case PointsModeEnum.GameObjectsTransform:
									...
									for (var i = 0; i < pointsGameObjects.Length; i++)
										Convert(pointsGameObjects[i], oldMode, pointsMode, provider);
									break;
							}
						...
						if (toRemove != null)
						{
							...
							for (var i = 0; i < toRemove.Length; i++)
							{
								...
								if (destroyer != null)
									destroyer(pointComponent);
								else
									DestroyIt(useComponents ? (Object)pointComponent : ((BGCurvePointGO)pointComponent).gameObject);
							}
						}
					}
		...
		private static IBGCurvePointI Convert(IBGCurvePointI point, PointsModeEnum from, PointsModeEnum to, Func<IBGCurvePointI> provider)
		{
			IBGCurvePointI result;
			switch (from)
			{
				...
				case PointsModeEnum.Components:
					...
					result = to switch
					{
						PointsModeEnum.Inlined => ((BGCurvePointComponent)point).Point,//---------- To inline
						PointsModeEnum.GameObjectsNoTransform or PointsModeEnum.GameObjectsTransform => ConvertInlineToGo(((BGCurvePointComponent)point).Point, to, provider),//---------- To GO
						_ => throw new ArgumentOutOfRangeException("to", to, null),
					};
					break;
				case PointsModeEnum.GameObjectsNoTransform:
				case PointsModeEnum.GameObjectsTransform:
			...
			if (pointTransform != null)
			{
				...
				if (from != PointsModeEnum.Inlined)
					reference = BGCurveReferenceToPoint.GetReferenceToPoint(point);

				if (to != PointsModeEnum.Inlined)
				{
					//not inlined
					if (reference == null)
						reference = pointTransform.gameObject.AddComponent<BGCurveReferenceToPoint>();
		...
		private static BGCurvePointGO ConvertInlineToGo(BGCurvePoint point, PointsModeEnum to, Func<IBGCurvePointI> provider)
		{
			BGCurvePointGO pointGO;
			if (provider != null)
				pointGO = (BGCurvePointGO)provider();
			else
			{
		...
		private static BGCurvePoint ConvertGoToInline(BGCurvePointGO pointGO, PointsModeEnum @from)
		{
			...
			switch (@from)
			{
				case PointsModeEnum.GameObjectsNoTransform:
					...
					break;
				case PointsModeEnum.GameObjectsTransform:
					var transform = pointGO.PointTransform ?? pointGO.Curve.transform;
			...
			if (pointGO.Curve.FieldsCount > 0)
				result.PrivateValuesForFields = pointGO.PrivateValuesForFields;
		...
		private void FireFinalEvent()
		{
			...
			if (!transformChanged && immediateChangeEvents && !forceUpdateEveryFrame)
				return;
			...
			if (!transformChanged && !changed && !forceUpdateEveryFrame)
				return;

			if (changed)
				FireChange(UseEventsArgs ? BGCurveChangedArgs.GetInstance(this, lastEventType, lastEventMessage) : null, true);
			else if (transformChanged)
				FireChange(UseEventsArgs ? BGCurveChangedArgs.GetInstance(this, BGCurveChangedArgs.ChangeTypeEnum.CurveTransform, EventTransform) : null, true);
			else
				FireChange(UseEventsArgs ? BGCurveChangedArgs.GetInstance(this, BGCurveChangedArgs.ChangeTypeEnum.Curve, EventForcedUpdate) : null, true);
		...
		private void AddFields(PointsModeEnum pointsMode, IBGCurvePointI[] addedPoints)
		{
		...
		private BGCurvePointField AddField(string name, BGCurvePointField.TypeEnum type, Func<BGCurvePointField> provider = null)
		{
			...
			if (PointsCount > 0)
			{
				...
				foreach (var point in points)
					BGCurvePoint.PrivateFieldAdded(field, GetFieldsValues(point, pointsMode));
			}
		...
		private void DeleteField(BGCurvePointField field, Action<BGCurvePointField> destroyer = null)
		{
			...
			if (deletedIndex < 0 || deletedIndex >= fields.Length)
				throw new UnityException("Unable to remove a fields. Invalid index: " + deletedIndex);
		...
		{
			private readonly Dictionary<string, int> fieldName2Index = new();
			...
			{
				if (fieldName2Index.TryGetValue(name, out var index))
					return index;
			...
			public void Update(BGCurvePointField[] fields)
			{
				...
				foreach (var field in fields)
				{
					...
					{
						BGCurvePointField.TypeEnum.Bool => boolIndex++,
						BGCurvePointField.TypeEnum.Int => intIndex++,
						BGCurvePointField.TypeEnum.Float => floatIndex++,
						BGCurvePointField.TypeEnum.Vector3 => vector3Index++,
						BGCurvePointField.TypeEnum.Bounds => boundsIndex++,
						BGCurvePointField.TypeEnum.Color => colorIndex++,
						BGCurvePointField.TypeEnum.String => stringIndex++,
						BGCurvePointField.TypeEnum.Quaternion => quaternionIndex++,
						BGCurvePointField.TypeEnum.AnimationCurve => animationCurveIndex++,
						BGCurvePointField.TypeEnum.GameObject => gameObjectIndex++,
						BGCurvePointField.TypeEnum.Component => componentIndex++,
						BGCurvePointField.TypeEnum.BGCurve => bgCurveIndex++,
						BGCurvePointField.TypeEnum.BGCurvePointComponent => bgCurvePointComponentIndex++,
						BGCurvePointField.TypeEnum.BGCurvePointGO => bgCurvePointGOIndex++,
						_ => throw new UnityException("Unknown type " + field.Type),
					};
