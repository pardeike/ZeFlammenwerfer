...
namespace BansheeGz.BGSpline.Curve
{
	...
	/// </summary>
	public class BGCurveBaseMath : IBGCurveMathI, IDisposable
	{
		#region events
		...
		protected readonly List<SectionInfo> cachedSectionInfos = new();

		// pools for sections
		protected readonly List<SectionInfo> poolSectionInfos = new();
		// pools for points
		protected readonly List<SectionPointInfo> poolPointInfos = new();
		...
		public int PointsCount
		{
			get
			{
				...
				for (var i = 0; i < length; i++)
					count += cachedSectionInfos[i].PointsCount;
				return count;
			}
		}
		...
		{
			BinarySearchByDistance(distance, out var position, out tangent, true, true);
		...
		public virtual Vector3 CalcByDistance(Field field, float distance, bool useLocal = false)
		{
			...
			BinarySearchByDistance(distance, out var position, out var tangent, calcPosition, !calcPosition);
		...
		public Vector3 CalcPositionByClosestPoint(Vector3 point, bool skipSectionsOptimization = false, bool skipPointsOptimization = false)
		{
			...
			return closestPointCalculator.CalcPositionByClosestPoint(point, out var _, out var _, skipSectionsOptimization, skipPointsOptimization);
		}
		...
		{
			closestPointCalculator ??= new BGCurveCalculatorClosestPoint(this);
			return closestPointCalculator.CalcPositionByClosestPoint(point, out distance, out var _, skipSectionsOptimization, skipPointsOptimization);
		}
		...
		public Vector3 GetControlFirst(int pointIndex)
		{
			...
			if (pointIndex == 0)
				return curve.Closed ? cachedSectionInfos[count - 1].OriginalToControl : curve[0].ControlFirstWorld;
		...
		public Vector3 GetControlSecond(int pointIndex)
		{
			...
			if (pointIndex == count)
				return curve.Closed ? cachedSectionInfos[count - 1].OriginalFromControl : curve[pointIndex].ControlSecondWorld;
		...
		public Bounds GetBoundingBox(int _, SectionInfo section)
		{
		...
		{
			ChangeRequested?.Invoke(this, null);
			...
			if (currentSectionsCount != sectionsCount)
			{
				if (currentSectionsCount < sectionsCount)
				{
					...
					for (var i = poolCount - 1; i >= 0 && toAddCount > 0; i--, toAddCount--)
						cachedSectionInfos.Add(poolSectionInfos[i]);

					//pool was used
					var usedPoolCount = toAddBeforePoolCount - toAddCount;
					if (usedPoolCount != 0)
						poolSectionInfos.RemoveRange(poolSectionInfos.Count - usedPoolCount, usedPoolCount);
				...
				else
				{
					...
					for (var i = sectionsCount; i < currentSectionsCount; i++)
						poolSectionInfos.Add(cachedSectionInfos[i]);
			...
			for (var i = 0; i < pointsCount - 1; i++)
				CalculateSection(i, cachedSectionInfos[i], i == 0 ? null : cachedSectionInfos[i - 1], curve[i], curve[i + 1]);
			...
			if (curve.Closed)
			{
				...
				if (cacheTangent)
					AdjustBoundaryPointsTangents(cachedSectionInfos[0], lastSection);
			}
		...
		protected virtual void CalculateSection(int index, SectionInfo section, SectionInfo prevSection, IBGCurvePointI @from, IBGCurvePointI to)
		{
			...
			if (Reset(section, @from, to, pointsCount) || Curve.SnapType == BGCurve.SnapTypeEnum.Curve)
			{
				if (straightAndOptimized)
				{
					...
					//tangents
					if (cacheTangent)
						startPoint.Tangent = endPoint.Tangent = (endPoint.Position - startPoint.Position).normalized;
				}
				else
				{
		...
		private void AdjustBoundaryPointsTangents(SectionInfo section, SectionInfo prevSection)
		{
			if (IsUseDistanceToAdjustTangents(section, prevSection))
			{
				...
				if (Math.Abs(overallDistance) < BGCurve.Epsilon)
					return;
		...
		protected virtual bool Reset(SectionInfo section, IBGCurvePointI @from, IBGCurvePointI to, int pointsCount)
		{
  ...
  ---> iii)   = ah
		...
		protected virtual void CalculateSplitSection(SectionInfo section, IBGCurvePointI @from, IBGCurvePointI to)
		{
			...
			if (!noControls && !bothControls && controlFromAbsent)
				control1 = control2;
			...
			if (noControls)
			{
				...
				if (cacheTangent)
					fromToTangentWorld = (toPos - fromPos).normalized;
				...
				for (var i = 1; i < parts; i++)
				{
					...
					if (cacheTangent)
					{
						if (config.UsePointPositionsToCalcTangents)
						{
							...
							point.Tangent = fromToTangentWorld;
					}
				...
				if (cacheTangent)
					firstPoint.Tangent = lastPoint.Tangent = fromToTangentWorld;
			}
			else
			{
				//for tangents
		...
		protected int FindSectionIndexByDistance(float distance)
		{
			...
			while (low < high)
			{
				...
				//success
				if (distance >= item.DistanceFromStartToOrigin && distance <= item.DistanceFromEndToOrigin)
					break;

				if (distance < item.DistanceFromStartToOrigin)
					high = mid;
				else
					low = mid + 1;
		...
		protected void Resize(List<SectionPointInfo> points, int size)
		{
			...
			if (pointCount < size)
			{
				...
				for (var i = pointCount; i < size; i++)
					points.Add(poolIndex >= 0 ? poolPointInfos[poolIndex--] : new SectionPointInfo());

				//pool was used
				if (poolIndex != poolPointInfos.Count - 1)
					poolPointInfos.RemoveRange(poolIndex + 1, poolPointInfos.Count - 1 - poolIndex);
			}
			else
			{
				//we need less points
				for (var i = size; i < pointCount; i++)
		...
		public class SectionInfo
		{
			...
			protected internal readonly List<SectionPointInfo> points = new();
			...
			protected internal bool Reset(IBGCurvePointI fromPoint, IBGCurvePointI toPoint, int pointsCount, bool skipCheck)
			{
			...
			public int FindPointIndexByDistance(float distanceWithinSection)
			{
				...
				while (low < high)
				{
					...
					//success
					if (!(distanceWithinSection < item.DistanceToSectionStart) && (mid == pointsCountMinusOne || points[mid + 1].DistanceToSectionStart >= distanceWithinSection))
						break;

					if (distanceWithinSection < item.DistanceToSectionStart)
						high = mid;
					else
						low = mid + 1;
			...
			public void CalcByDistance(float distanceWithinSection, out Vector3 position, out Vector3 tangent, bool calculatePosition, bool calculateTangent)
			{
				...
				if (points.Count == 2)
				{
					...
					else
					{
						...
						var second = points[1];
						if (calculatePosition)
							position = Vector3.Lerp(first.Position, second.Position, ratio);
						if (calculateTangent)
							tangent = Vector3.Lerp(first.Tangent, second.Tangent, ratio);
					}
				}
				else
				{
					...
					else
					{
						...
						if (calculatePosition)
							position = Vector3.Lerp(targetPoint.Position, nextPoint.Position, ratio);
						if (calculateTangent)
							tangent = Vector3.Lerp(targetPoint.Tangent, nextPoint.Tangent, ratio);
					}
				}
			}
		}
		...
		public class SectionPointInfo
		{
			...
			internal Vector3 GetField(Field field)
			{
				var result = field switch
				{
					...
					_ => throw new UnityException("Unknown field=" + field),
				};
				return result;
			}
