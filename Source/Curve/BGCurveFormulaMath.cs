...
namespace BansheeGz.BGSpline.Curve
{
	...
	public class BGCurveFormulaMath : BGCurveBaseMath
	{
		...
		protected override void CalculateSplitSection(SectionInfo section, IBGCurvePointI @from, IBGCurvePointI to)
		{
			...
			if (noControls)
			{
				...
				if (cacheTangent)
					fromToTangentWorld = (to.PositionWorld - @from.PositionWorld).normalized;
			}
					...
					if (cacheTangent)
					{
						if (config.UsePointPositionsToCalcTangents)
						{
							...
							if (i != 0)
							{
								...
								if (i == config.Parts)
									point.Tangent = prevPoint.Tangent;
							}
						}
						else
						{
