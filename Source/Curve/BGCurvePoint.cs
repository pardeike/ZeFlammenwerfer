...
namespace BansheeGz.BGSpline.Curve
{
	...
	[Serializable]
	public class BGCurvePoint : IBGCurvePointI
	{
		...
		//point's curve
		[SerializeField] private readonly BGCurve curve;
		...
		public FieldsValues PrivateValuesForFields
		{
			get
			{
				if (fieldsValues == null || fieldsValues.Length < 1 || fieldsValues[0] == null)
					fieldsValues = new[] { new FieldsValues() };
		...
		{
			get { return (pointTransform ?? curve.transform).TransformVector(controlFirstLocal); }
			set
			{
				var transform = pointTransform ?? curve.transform;
		...
		{
			get { return (pointTransform ?? curve.transform).TransformVector(controlSecondLocal); }
			set
			{
				var transform = pointTransform ?? curve.transform;
		...
		public Vector3 Get(FieldEnum field)
		{
			...
			{
				//position
				FieldEnum.PositionWorld => PositionWorld,
				FieldEnum.PositionLocal => positionLocal,
				//first control
				FieldEnum.ControlFirstWorld => ControlFirstWorld,
				FieldEnum.ControlFirstLocal => controlFirstLocal,
				//second control
				FieldEnum.ControlSecondWorld => ControlSecondWorld,
				_ => controlSecondLocal,
			};
			return result;
		}
		...
		{
			private static readonly Dictionary<Type, Func<FieldsValues, int, object>> type2fieldGetter = new();
			private static readonly Dictionary<Type, Action<FieldsValues, int, object>> type2fieldSetter = new();
			private static readonly Dictionary<BGCurvePointField.TypeEnum, Type> type2Type = new();
			...
			{
				if (!type2fieldGetter.TryGetValue(type, out var getter))
			...
			{
				if (!type2fieldSetter.TryGetValue(type, out var setter))
					throw new UnityException("Unsupported type for a field, type= " + type);
