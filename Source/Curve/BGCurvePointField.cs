...
#pragma warning restore 0414
		...
		public static string CheckName(BGCurve curve, string name, bool throwException = false)
		{
			...
			if (string.IsNullOrEmpty(name))
				error = "Field's name can not be null";
			else if (name.Length > 16)
				error = "Name should be 16 chars max. Current name has " + name.Length + " chars.";
			else
			{
				var firstChar = name[0];
				if (!((firstChar >= 'A' && firstChar <= 'Z') || (firstChar >= 'a' && firstChar <= 'z')))
					error = "Name should start with a English letter.";
				else
				{
					...
					if (error == null && curve.HasField(name))
