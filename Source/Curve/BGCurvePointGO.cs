...
namespace BansheeGz.BGSpline.Curve
{
	...
	public class BGCurvePointGO : MonoBehaviour, IBGCurvePointI
	{
		#region fields
		...
		public BGCurvePoint.FieldsValues PrivateValuesForFields
		{
			get
			{
				if (fieldsValues == null || fieldsValues.Length < 1 || fieldsValues[0] == null)
					fieldsValues = new[] { new BGCurvePoint.FieldsValues() };
		...
		public Vector3 PositionLocal
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => positionLocal,
					BGCurve.PointsModeEnum.GameObjectsTransform => curve.transform.InverseTransformPoint(transform.position),
					_ => throw WrongMode(),
				};
			}
		...
		public Vector3 PositionLocalTransformed
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform.TransformPoint(positionLocal) - curve.transform.position,
					BGCurve.PointsModeEnum.GameObjectsTransform => transform.position - curve.transform.position,
					_ => throw WrongMode(),
				};
			}
		...
		public Vector3 PositionWorld
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform.TransformPoint(positionLocal),
					BGCurve.PointsModeEnum.GameObjectsTransform => transform.position,
					_ => throw WrongMode(),
				};
			}
		...
		public Vector3 ControlFirstWorld
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform.TransformPoint(new Vector3(positionLocal.x + controlFirstLocal.x, positionLocal.y + controlFirstLocal.y, positionLocal.z + controlFirstLocal.z)),
					BGCurve.PointsModeEnum.GameObjectsTransform => transform.position + transform.TransformVector(controlFirstLocal),
					_ => throw WrongMode(),
				};
			}
			set
			{
				Vector3 localPos;
				if (pointTransform != null)
					localPos = pointTransform.InverseTransformVector(value - pointTransform.position);
				else
				{
					localPos = Curve.PointsMode switch
					{
						BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform.InverseTransformPoint(value) - PositionLocal,
						BGCurve.PointsModeEnum.GameObjectsTransform => transform.InverseTransformVector(value - transform.position),
						_ => throw WrongMode(),
					};
				}
		...
		public Vector3 ControlSecondWorld
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform.TransformPoint(new Vector3(positionLocal.x + controlSecondLocal.x, positionLocal.y + controlSecondLocal.y, positionLocal.z + controlSecondLocal.z)),
					BGCurve.PointsModeEnum.GameObjectsTransform => transform.position + transform.TransformVector(controlSecondLocal),
					_ => throw WrongMode(),
				};
			}
			set
		...
		private Transform TargetTransform
		{
			get
			{
				...
				return Curve.PointsMode switch
				{
					BGCurve.PointsModeEnum.GameObjectsNoTransform => curve.transform,
					BGCurve.PointsModeEnum.GameObjectsTransform => transform,
					_ => throw WrongMode(),
				};
			}
		}
			...
			if (point != null)
			{
				...
				switch (pointsMode)
				{
					...
					case BGCurve.PointsModeEnum.GameObjectsTransform:
						...
						var targetTransform = pointTransform ?? transform;
			...
			else
			{
				...
				switch (pointsMode)
				{
					case BGCurve.PointsModeEnum.GameObjectsNoTransform:
					{
						...
						//transformed locals are always the same
						targetTransform = pointTransform ?? curve.transform;
						break;
					}
					case BGCurve.PointsModeEnum.GameObjectsTransform:
					{
						...
						//transformed locals are always the same
						targetTransform = pointTransform ?? transform;
						break;
					}
					default:
						throw new ArgumentOutOfRangeException("pointsMode", pointsMode, null);
